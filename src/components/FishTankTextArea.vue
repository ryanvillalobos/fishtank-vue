<template>
  <div :class="['text-area',{ 'error': !!errorMessage }, orientation]">
    <div v-if="label" class="label-wrapper">
      <label :for="`textarea-${identifier}-id`" class="label">
      ...
      <span class="auxillary-slot">
        <!-- @slot Slot for auxillary content -->
        <slot name="auxillary" />
      </span>
    </div>
    <div class="input-wrapper">
      <span v-if="$slots.leftIcon" class="left-icon">
        <!-- @slot Slot for lef icon -->
        <slot name="leftIcon" />
      </span>
      <!--eslint-disable-->
      <textarea
      ...
      />
      <!--eslint-enable-->
      <p ref="falseTextarea" class="false-text-area">&nbsp;{{ value }}</p>
      <span v-if="showRightIcon" class="right-icon">
        <!-- @slot Slot for right icon -->
        <slot name="rightIcon">
          <span class="clear" @click="clearText">
            <CloseIcon />
          </span>
        </slot>
      </span>
      <!-- @slot slot for below text area -->
      <slot name="below" />
    </div>

    <div v-if="errorMessage" id="error-block" :class="{'error-text':errorMessage}">
...
@Component({
  ...
  mixins: [a11y, textInputMixin, orientation],
  inheritAttrs: false
})
export default class FishTankTextArea extends Vue {
  /**
   * Textarea type input max-height
   */
  @Prop({
    type: Number,
    default: null,
  ...
  checkError() {
    if (
      ...
      this.$props.error.fullMessage != undefined ||
      this.$props.error.fullMessage != null
    ) {
      return;
    } else {
      this.reset();
    ...
    setTimeout(() => {
      this.$nextTick(() => {
        if (
          ...
          this.$props.maxheight <
            (this.$refs.falseTextarea as HTMLDivElement).clientHeight
        ) {
        ...
        if (this.$refs.falseTextarea !== undefined) {
          ...
          this.textAreafalseHeight = (this.$refs
            .falseTextarea as HTMLDivElement).clientHeight;
        }
